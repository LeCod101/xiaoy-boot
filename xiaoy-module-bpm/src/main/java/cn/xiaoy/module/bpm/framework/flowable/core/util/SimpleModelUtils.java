package cn.xiaoy.module.bpm.framework.flowable.core.util;
...
import cn.xiaoy.module.bpm.controller.admin.definition.vo.model.simple.BpmSimpleModelNodeVO.ConditionGroups;
...
import cn.xiaoy.module.bpm.framework.flowable.core.listener.BpmCopyTaskDelegate;
import cn.xiaoy.module.bpm.framework.flowable.core.listener.BpmTriggerTaskDelegate;
import cn.xiaoy.module.bpm.service.task.listener.BpmCallActivityListener;
import cn.xiaoy.module.bpm.service.task.listener.BpmUserTaskListener;
...
import static cn.xiaoy.module.bpm.framework.flowable.core.enums.BpmnModelConstants.*;
...
public class SimpleModelUtils {
    ...
    private static void traverseBranchNodeToBuildSequenceFlow(Process process, BpmSimpleModelNodeVO node, String targetNodeId) {
        ...
        // TODO @小Y 路由分支没有conditionNodes 这里注释会影响吗？@小Y：一起帮忙瞅瞅！
//        Assert.notEmpty(conditionNodes, "分支节点的条件节点不能为空");
    ...
    private static SequenceFlow buildBpmnSequenceFlow(String sourceId, String targetId,
        ...
        // TODO @小Y：如果 sequenceFlowId 不存在的时候，是不是要生成一个默认的 sequenceFlowId？ @小Y： 貌似不需要,Flowable 会默认生成；TODO @小Y：建议还是搞一个，主要是后续好排查问题。
        // TODO @小Y：如果 name 不存在的时候，是不是要生成一个默认的 name？ @小Y： 不需要生成默认的吧？ 这个会在流程图展示的， 一般用户填写的。不好生成默认的吧；TODO @小Y：建议还是搞一个，主要是后续好排查问题。
    ...
    private static class StartUserNodeConvert implements NodeConvert {
        ...
        public UserTask convert(BpmSimpleModelNodeVO node) {
            ...
            // 使用自动通过策略
            // TODO @小Y 复用了SKIP， 是否需要新加一个策略；TODO @小Y：【回复】是不是应该类似飞书，搞个草稿状态。待定；还有一种策略，不标记自动通过，而是首次发起后，第一个节点，自动通过；
    ...
    private static class ParallelBranchNodeConvert implements NodeConvert {
        ...
        /**
         * 并行分支使用包容网关。需要设置所有出口条件表达式的值为 true 。原因是，解决 https://docs.xiaoy.cn/m6GXh 反馈问题
         *
