package cn.xiaoy.module.bpm.service.task;
...
import cn.xiaoy.framework.common.util.collection.CollectionUtils;
import cn.xiaoy.framework.common.util.date.DateUtils;
import cn.xiaoy.framework.common.util.number.NumberUtils;
import cn.xiaoy.framework.common.util.object.ObjectUtils;
import cn.xiaoy.framework.common.util.object.PageUtils;
import cn.xiaoy.framework.datapermission.core.annotation.DataPermission;
import cn.xiaoy.framework.web.core.util.WebFrameworkUtils;
import cn.xiaoy.module.bpm.controller.admin.definition.vo.model.BpmModelMetaInfoVO;
import cn.xiaoy.module.bpm.controller.admin.task.vo.task.*;
import cn.xiaoy.module.bpm.convert.task.BpmTaskConvert;
import cn.xiaoy.module.bpm.dal.dataobject.definition.BpmFormDO;
import cn.xiaoy.module.bpm.dal.dataobject.definition.BpmProcessDefinitionInfoDO;
import cn.xiaoy.module.bpm.enums.definition.*;
import cn.xiaoy.module.bpm.enums.task.*;
import cn.xiaoy.module.bpm.framework.flowable.core.enums.BpmTaskCandidateStrategyEnum;
import cn.xiaoy.module.bpm.framework.flowable.core.enums.BpmnVariableConstants;
import cn.xiaoy.module.bpm.framework.flowable.core.util.BpmHttpRequestUtils;
import cn.xiaoy.module.bpm.framework.flowable.core.util.BpmnModelUtils;
import cn.xiaoy.module.bpm.framework.flowable.core.util.FlowableUtils;
import cn.xiaoy.module.bpm.service.comment.BpmCommentService;
import cn.xiaoy.module.bpm.service.definition.BpmFormService;
import cn.xiaoy.module.bpm.service.definition.BpmModelService;
import cn.xiaoy.module.bpm.service.definition.BpmProcessDefinitionService;
import cn.xiaoy.module.bpm.service.message.BpmMessageService;
import cn.xiaoy.module.bpm.service.message.dto.BpmMessageSendWhenTaskTimeoutReqDTO;
...
import static cn.xiaoy.module.bpm.enums.ErrorCodeConstants.*;
import static cn.xiaoy.module.bpm.framework.flowable.core.enums.BpmnModelConstants.START_USER_NODE_ID;
import static cn.xiaoy.module.bpm.framework.flowable.core.util.BpmnModelUtils.*;
...
//        }
        ...
        // TODO @小Y：https://docs.xiaoy.cn/MNzqp 【flowable bug】：taskCreatedAfter、taskCreatedBefore 拼接的是 OR
        if (ArrayUtil.isNotEmpty(pageVO.getCreateTime())) {
...
//        }
    ...
    public List<UserTask> getUserTaskListByReturn(String id) {
        ...
        // 2.3 过滤：只能退回到已经处理过的节点（排除审批未经过的节点）。相关 issue：https://localhost/xiaoy/issues/982
    ...
    @DataPermission(enable = false) // 关闭数据权限，避免查询不到用户数据。相关案例：https://localhost/xiaoy/xiaoy-cloud/issues/ID1UYA
    ...
    private Map<String, Object> validateAndSetNextAssignees(String taskDefinitionKey, Map<String, Object> variables, BpmnModel bpmnModel,
        ...
        for (UserTask nextFlowNode : nextFlowNodes) {
            ...
            if (ObjUtil.equals(candidateStrategy, BpmTaskCandidateStrategyEnum.APPROVE_USER_SELECT.getStrategy())) {
                // 特殊：如果当前节点已经存在审批人，则不允许覆盖。 例如并行节点后，设置的审批人自选节点。 https://docs.xiaoy.cn/daxv1
    ...
    public void rejectTask(Long userId, @Valid BpmTaskRejectReqVO reqVO) {
        ...
        // 疑问：为什么要标记未通过呢？
        // 回答：例如说 A 任务被向前加签除 B 任务时，B 任务被审批不通过，此时 A 会被取消。而 xiaoy-ui-admin-vue3 不展示“已取消”的任务，导致展示不出审批不通过的细节。
    ...
    public void returnTask(Long userId, BpmnModel bpmnModel, Task currentTask, FlowElement targetElement, BpmTaskReturnReqVO reqVO) {
        ...
        //    相关 issue：https://localhost/xiaoy/issues/1018
        // ③ moveActivityIdsToSingleActivityId 使用遇到问题， 相关 issue https://localhost/xiaoy/xiaoy-cloud/issues/IJM8MS
        ...
        // flowable 8.0.0  修复上面相关问题， 还修复了并行分支回退的问题 https://docs.xiaoy.cn/z4d9i。
        // ⑤ 使用 moveExecutionsToSingleActivityId 方法进行回退操作时，如果是多实例的用户任务【小Y用户任务默认为多实例】，不会删除多实例任务的根数据 ACT_RU_EXECUTION
    ...
    public void delegateTask(Long userId, BpmTaskDelegateReqVO reqVO) {
        ...
        // 特殊：如果已经被委派（owner 非空），则不需要更新 owner：https://localhost/xiaoy/xiaoy-cloud/issues/ICJ153
    ...
    public void transferTask(Long userId, BpmTaskTransferReqVO reqVO) {
        ...
        // 3.1 设置任务所有人 (owner) 为原任务的处理人 (assignee)
        // 特殊：如果已经被转派（owner 非空），则不需要更新 owner：https://localhost/xiaoy/xiaoy-cloud/issues/ICJ153
        if (StrUtil.isEmpty(task.getOwner())) {
            taskService.setOwner(taskId, task.getAssignee());
        }
    ...
    public void moveTaskToEnd(String processInstanceId, String reason) {
        ...
        // TODO 小Y：目前发现并行分支情况下，会存在这个情况，后续看看有没更好的方案；
    ...
    public void withdrawTask(Long userId, String taskId) {
        ...
        // TODO @小Y：是否选择升级flowable版本解决taskCreatedAfter、taskCreatedBefore问题，升级7.1.0可以；包括 todo 和 done 那边的查询哇？？？ 是的！
    ...
    public void processTaskCreated(Task task) {
        ...
        TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
             ...
             * 参见 <a href="https://localhost/xiaoy/xiaoy-cloud/issues/IB7V7Q">issue</a> 反馈
             */
            @Override
