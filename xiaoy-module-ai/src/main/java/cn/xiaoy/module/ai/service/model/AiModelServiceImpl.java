...
import cn.xiaoy.module.ai.controller.admin.model.vo.model.AiModelPageReqVO;
import cn.xiaoy.module.ai.controller.admin.model.vo.model.AiModelSaveReqVO;
import cn.xiaoy.module.ai.dal.dataobject.model.AiApiKeyDO;
import cn.xiaoy.module.ai.dal.dataobject.model.AiModelDO;
import cn.xiaoy.module.ai.dal.mysql.model.AiChatMapper;
...
//         return modelFactory.getOrCreateVectorStore(MilvusVectorStore.class, embeddingModel, metadataFields);
    ...
    public void getLLmProvider4Tinyflow(Tinyflow tinyflow, Long modelId) {
        ...
        AiPlatformEnum platform = AiPlatformEnum.validatePlatform(apiKey.getPlatform());
        switch (platform) {
            // TODO @小Y 考虑到未来不需要使用agents-flex 现在仅测试通义千问
            // TODO @小Y：【重要】是不是可以实现一个 SpringAiLlm，这样的话，内部全部用它就好了。只实现 chat 部分；这样，就把 flex 作为一个 agent 框架，内部调用，还是 spring ai 相关的。成本可能低一点？！
            case TONG_YI:
                ...
                // TODO @小Y：这个有点奇怪。。。如果一个链式里，有多个模型，咋整呀。。。
