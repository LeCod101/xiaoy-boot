package cn.xiaoy.module.iot.core.messagebus.config;

import cn.xiaoy.framework.mq.redis.core.RedisMQTemplate;
import cn.xiaoy.framework.mq.redis.core.job.RedisPendingMessageResendJob;
import cn.xiaoy.framework.mq.redis.core.job.RedisStreamMessageCleanupJob;
import cn.xiaoy.framework.mq.redis.core.stream.AbstractRedisStreamMessage;
import cn.xiaoy.framework.mq.redis.core.stream.AbstractRedisStreamMessageListener;
import cn.xiaoy.module.iot.core.messagebus.core.IotMessageBus;
import cn.xiaoy.module.iot.core.messagebus.core.kafka.IotKafkaMessageBus;
import cn.xiaoy.module.iot.core.messagebus.core.local.IotLocalMessageBus;
import cn.xiaoy.module.iot.core.messagebus.core.rabbitmq.IotRabbitMQMessageBus;
import cn.xiaoy.module.iot.core.messagebus.core.redis.IotRedisMessageBus;
import cn.xiaoy.module.iot.core.messagebus.core.rocketmq.IotRocketMQMessageBus;
...
public class IotMessageBusAutoConfiguration {
    ...
    @Configuration
    @ConditionalOnProperty(prefix = "xiaoy.iot.message-bus", name = "type", havingValue = "local", matchIfMissing = true)
    ...
    @Configuration
    @ConditionalOnProperty(prefix = "xiaoy.iot.message-bus", name = "type", havingValue = "rocketmq")
    ...
    @Configuration
    @ConditionalOnProperty(prefix = "xiaoy.iot.message-bus", name = "type", havingValue = "kafka")
    ...
    /**
     * 特殊：由于 XiaoyRedisMQConsumerAutoConfiguration 关于 Redis stream 的消费是动态注册，所以这里只能拷贝相关的逻辑！！！
     *
     * @see cn.xiaoy.framework.mq.redis.config.XiaoyRedisMQConsumerAutoConfiguration
     */
    @Configuration
    @ConditionalOnProperty(prefix = "xiaoy.iot.message-bus", name = "type", havingValue = "redis")
    ...
    @Configuration
    @ConditionalOnProperty(prefix = "xiaoy.iot.message-bus", name = "type", havingValue = "rabbitmq")
