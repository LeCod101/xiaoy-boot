...
or with uv
uv run --with simple-ddl-parser convertor.py postgres ../mysql/xiaoy.sql > ../postgresql/xiaoy.sql
uv run --with simple-ddl-parser convertor.py sqlserver ../mysql/xiaoy.sql > ../sqlserver/xiaoy.sql
uv run --with simple-ddl-parser convertor.py kingbase ../mysql/xiaoy.sql > ../kingbase/xiaoy.sql
uv run --with simple-ddl-parser convertor.py opengauss ../mysql/xiaoy.sql > ../opengauss/xiaoy.sql
uv run --with simple-ddl-parser convertor.py highgo ../mysql/xiaoy.sql > ../highgo/xiaoy.sql
uv run --with simple-ddl-parser convertor.py oracle ../mysql/xiaoy.sql > ../oracle/xiaoy.sql
uv run --with simple-ddl-parser convertor.py dm8 ../mysql/xiaoy.sql > ../dm/xiaoy-dm8.sql
"""
...
def load_and_clean(sql_file: str) -> str:
    ...
    # 相关 issue：https://docs.xiaoy.cn/96IFc 、https://docs.xiaoy.cn/rC3A3
...
def main():
    parser = argparse.ArgumentParser(description="小Y系统数据库转换工具")
    parser.add_argument(
        "type",
        type=str,
        help="目标数据库类型",
