...
import cn.xiaoy.module.report.controller.admin.goview.vo.data.GoViewDataRespVO;
...
public class GoViewDataServiceImplTest extends BaseDbUnitTest {
    ...
    public void testGetDataBySQL() {
        ...
        when(sqlRowSet.getObject("name")).thenReturn("小Y系统").thenReturn("小Y");
        ...
        assertEquals("小Y系统", dataBySQL.getSource().get(0).get("name"));
        assertEquals(2, dataBySQL.getSource().get(1).size());
        assertEquals(2L, dataBySQL.getSource().get(1).get("id"));
        assertEquals("小Y", dataBySQL.getSource().get(1).get("name"));
    }
