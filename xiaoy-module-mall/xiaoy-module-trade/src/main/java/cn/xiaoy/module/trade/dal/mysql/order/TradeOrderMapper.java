package cn.xiaoy.module.trade.dal.mysql.order;
...
public interface TradeOrderMapper extends BaseMapperX<TradeOrderDO> {
    ...
    // TODO @小Y：如果用 map 返回，要不这里直接用 TradeOrderSummaryRespVO 返回？也算合理，就当  sql 查询出这么个玩意~~
    ...
    default PageResult<TradeOrderDO> selectPage(AppTradeOrderPageReqVO reqVO, Long userId) {
        return selectPage(reqVO, new LambdaQueryWrapperX<TradeOrderDO>()
                ...
                .orderByDesc(TradeOrderDO::getId)); // TODO 小Y：未来不同的 status，不同的排序
    }
