...
import cn.xiaoy.module.system.mq.message.mail.MailSendMessage;
import cn.xiaoy.module.system.mq.producer.mail.MailProducer;
...
//                .setFrom("奥特曼 <ydym_test@163.com>")
                ...
                .setAuth(true).setUser("ydym_test@163.com").setPass("YOUR_MAIL_PASSWORD"); // 登录账号密码
    ...
    public void testDoSendMail_success() {
        try (final MockedStatic<MailUtil> mailUtilMock = mockStatic(MailUtil.class)) {
            // 准备参数
            MailSendMessage message = randomPojo(MailSendMessage.class, o -> o.setNickname("小Y"));
            // mock 方法（获得邮箱账号）
            ...
            mailUtilMock.when(() -> MailUtil.send(
                            argThat(mailAccount -> {
                                assertEquals("小Y <7685@qq.com>", mailAccount.getFrom());
