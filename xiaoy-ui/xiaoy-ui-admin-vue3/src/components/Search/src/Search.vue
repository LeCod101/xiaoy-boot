...
<template>
  <!-- update by 小Y：class="-mb-15px" 用于降低和 ContentWrap 组件的底部距离，避免空隙过大 -->
  <Form
    :inline="inline"
    :is-col="isCol"
    :is-custom="false"
  ...
  >
    <template #action>
      <div v-if="layout === 'inline'">
        <!-- update by 小Y：去除搜索的 type="primary"，颜色变淡一点 -->
        <el-button v-if="showSearch" @click="search">
          <Icon class="mr-5px" icon="ep:search" /> {{ t('common.query') }}
        </el-button>
        <!-- update by 小Y：将 icon="ep:refresh-right" 修改成 icon="ep:refresh"，和 小Y系统 搜索保持一致  -->
